...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver {
    ...
    protected function get_true_false_input($field_info, $value) {
        ...
        $input .= "<input class='switch-input' id='field-{$field_info->name}-true' type=\"radio\" name=\"{$field_info->name}\" value=\"1\" $checked />
			 <label for='field-{$field_info->name}-true' class='switch-label switch-label-off'>$true_string</label>";
        ...
        $input .= "<input class='switch-input' id='field-{$field_info->name}-false' type=\"radio\" name=\"{$field_info->name}\" value=\"0\" $checked />
			 <label for='field-{$field_info->name}-false' class='switch-label switch-label-on'>$false_string</label>";
...
class Grocery_CRUD extends grocery_CRUD_States {
    ...
    protected function get_read_input_fields($field_values = null) {
        ...
        foreach ($fields as $field_num => $field) {
            ...
            if (isset($field_info->db_type) && ($field_info->db_type == 'tinyint' || ($field_info->db_type == 'int' && $field_info->db_max_length == 1))) {
                $field_value = $this->get_true_false_input($field_info, $field_values->{$field->field_name});
            } else {
                $field_value = !empty($field_values) && isset($field_values->{$field->field_name}) ? $field_values->{$field->field_name} : null;
            }
            if (!isset($this->callback_read_field[$field->field_name])) {
    ...
    public function set_field_upload($field_name, $upload_dir = '', $allowed_file_types = '') {
        ...
        if (!is_dir(FCPATH . $upload_dir)) {
            mkdir(FCPATH . $upload_dir, 0775, true);
...
if (defined('CI_VERSION')) {
    ...
    class grocery_CRUD_Form_validation extends CI_Form_validation {
        ...
        public function phone_number($str) {
            return (bool) preg_match('/^00[0-9]{2,3}[0-9]{9}$/', $str);
        }
